...
struct ip_fw 
{
	/* 	src, dst, src_mask, dst_mask 按网络字节序存放 */
	struct ip_fw  *fw_next;			/* Next firewall on chain */
	struct in_addr fw_src, fw_dst;		/* Source and destination IP addr */
	struct in_addr fw_smsk, fw_dmsk;	/* Mask for src and dest IP addr */
	struct in_addr fw_via;			/* IP address of interface "via" 接收接口的IP地址*/
...
#define IP_FW_MAX_PORTS	10      		/* A reasonable maximum */
	/* 每个元素表示一个端口号，fw_nsp个源端口，fw_ndp个目标端口 */
...
#define IP_FW_CHK_BLK    (IP_FW_BASE_CTL+2)			// 检查阻塞规则链表中规则的有效性
#define IP_FW_CHK_FWD    (IP_FW_BASE_CTL+3)			// 检查转发规则链表中规则的有效性
#define IP_FW_DEL_BLK    (IP_FW_BASE_CTL+4)
#define IP_FW_DEL_FWD    (IP_FW_BASE_CTL+5)		
#define IP_FW_FLUSH_BLK  (IP_FW_BASE_CTL+6)			// 删除阻塞规则链表中的所有规则
#define IP_FW_FLUSH_FWD  (IP_FW_BASE_CTL+7)		// 删除转发规则链表中的所有规则
#define IP_FW_ZERO_BLK   (IP_FW_BASE_CTL+8)			// 清空阻塞规则链表中的统计信息
#define IP_FW_ZERO_FWD   (IP_FW_BASE_CTL+9)		// 清空转发规则链表中的统计信息
#define IP_FW_POLICY_BLK (IP_FW_BASE_CTL+10)		// 设置阻塞规则的默认策略
#define IP_FW_POLICY_FWD (IP_FW_BASE_CTL+11)		// 设置转发规则的默认策略
...
/* 仅对本机原始发送的数据报以及最终的端是本机的数据包进行检测
 * 根据具体规则对数据包进行处理
 * 如果规则没有定义具体处理方式，就按照全局变量ip_fw_blk_policy的值进行取舍
 */
extern struct ip_fw *ip_fw_blk_chain;
/* 适用于每个需经本机转发的数据包
 * 根据具体规则对数据包紧凑型处理
 * 如果规则没有定义具体处理方式，就按照全局变量ip_fw_fwd_policy的值进行取舍
 */
