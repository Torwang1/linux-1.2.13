...
/* socket::flag标志选项	*/
#define SO_ACCEPTCON	(1<<16)		/* performed a listen	监听套接字标记		*/
#define SO_WAITDATA	(1<<17)		/* wait data to read	读阻塞	*/
#define SO_NOSPACE		(1<<18)		/* no space to write	写阻塞	*/
...
struct socket {
  ...
  void			*data;		/* protocol data					*
  							 * 保存内核私有的数据成员，对于 	*	
  							 * UNIX域，data指向unix_proto_data	*
  							 * 数据结构，对于INET域，指向sock数 *
  							 * 据结构。							*/
  struct socket		*conn;		/* server socket connected to	在INET域中，该字段为NULL	*/
  struct socket		*iconn;		/* incomplete client conn.s		在INET域中，该字段为NULL	*/
  struct socket		*next;
  struct wait_queue	**wait;		/* ptr to place to wait on		 	*
  								 * 在sock_alloc()中wait被指向inode::i_wait等待队列*/
  struct inode		*inode;
...
/* 
 * 协议簇(域)的抽象接口，为上层BSD层提供统一接口
 *
 * 每个协议簇("域")都会定义struct proto_ops结构
 * 如:unix_proto_ops   inet_proto_ops 	ipx_proto_ops
 * 目的是把BSD层的操作转化成协议簇的操作
 */
struct proto_ops {
  int	family;
...
/*
 * 唤醒等待在套接字上的进程,通常发生在套接字状态发生改变时
 * 例: accept接收新连接，但监听套接字上没有就绪链接，那么调用进程将阻塞在监听套接字上
 * 当监听套接字状态发生变化的时候，由系统调用该函数，来唤醒等待进程
 */
