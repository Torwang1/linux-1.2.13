...
static struct sk_buff *tcp_find_established(struct sock *s)
{
	...
	do
	{
		/* 返回sock->state为TCP_ESTABLISHED/TCP_SYN_SEND/TCP_SYN_RECV/TCP_FIN_WAIT_1状态的套接字	*/
		/**
		 * TCP_ESTABLISHED:	期待的正常状态
		 * TCP_SYN_RECV:	也可以，实现把未完成的握手也放在receive_queue队列中
		 * TCP_FIN_WAIT_1:	也可以，监听套接字没来得及处理连接请求，客户端就已经close了连接，这时是可以出现这种情况的
		 * TCP_SYN_SEND: 	????????无法解释，为什么会出现这种状态?????????
		 **/
...
static int tcp_select(struct sock *sk, int sel_type, select_table *wait)
{
	...
	case SEL_IN:
		...
		if (sk->urg_seq != sk->copied_seq ||		// 接收缓存中有紧急数据尚未被用户进程读取
 ...
 */
/**
 * 因为锁问题，可能出现多个paritally complete buffer
 * tcp_dequeue_partial有问题，应该是每次返回一个partial buffer,但代码实现中一次返回了一个队列
 **/
 ...
 *	listening.
 *
 *  处理客户端的SYN请求，创建sock结构，发送SYN+ACK
 */
		 ...
		 struct options *opt, struct device *dev, unsigned long seq)
{
	...
	if (!sk->dead) 
	{
  		sk->data_ready(sk,0);		// 接收到连接请求，唤醒等待进程
	}
	else 
	{
		if(sk->debug)
	...
	newsk->state = TCP_SYN_RECV;		// 设置接收的新连接状态为TCP_SYN_RECV
	newsk->timeout = 0;
	newsk->ip_xmit_timeout = 0;
	newsk->write_seq = seq; 					// 本地初始化的序列号
	newsk->window_seq = newsk->write_seq;	// 初始发送窗口边界
	newsk->rcv_ack_seq = newsk->write_seq;	// 已被确认字节的下一个字节编号
	newsk->urg_data = 0;
	newsk->retransmits = 0;
	newsk->linger=0;
	newsk->destroy = 0;
	...
	put_sock(newsk->num,newsk);		// 给newsk绑定与监听套接字相同的端口号
	...
	newsk->acked_seq = skb->h.th->seq + 1;	// 确认号为对端序列号+1(规定:SYN要消耗一个字节的seq)
	newsk->copied_seq = skb->h.th->seq + 1; 	// 接收缓存中没有未读取的数据
	newsk->socket = NULL;
	...
	rt=ip_rt_route(saddr, NULL,NULL);	// 用对端地址查找rt，并使用rt信息初始化newsk
	...
	newsk->prot->queue_xmit(newsk, ndev, buff, 0);			// 直接使用不完整的newsock发送相应
	...
	/* 将包含请求的skb可以引用到newsk，挂到监听套接字的接收队列上	*/
