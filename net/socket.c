 ...
 */
/* 普通文件操作函数集
 * 系统中每个文件对应一个file结构，对应一个file_operations结构
 *
 * 通过这个结构体对socket做普通文件的操作
 */
...
struct socket *sock_alloc(void)
{
	...
	sock->iconn = NULL;
	sock->next = NULL;
	sock->wait = &inode->i_wait; 	/* 普通文件操作有可能造成进程阻塞，sock使用inode的等待队列保存阻塞进程的链表 */
...
static int sock_select(struct inode *inode, struct file *file, int sel_type, select_table * wait)
{
	 ...
	 */
	/* 检查socket是否绑定到具体协议簇，协议簇是否支持select操作	*/
 ...
 */
/*   
 * @on  1: 表示创建struct fasync_struct    0: 表示删除struct fasync_struct	
 * 
 * struct fasync_struct结构定义在sock结构中，被进程拥有
 */
static int sock_fasync(struct inode *inode, struct file *filp, int on)
{
...
/**
 * 发送"信号 SIGIO",唤醒异步等待进程 				
 * 当套接字状态发生改变时，通知相应进程(等待在异步通知队列中的进程)
 */
	...
	switch (how)
	{
		...
		case 1:	
			if (!(sock->flags & SO_WAITDATA))
				kill_fasync(sock->fasync_list, SIGIO);	// SIGIO默认处理方式是忽略/终止
			break;
		case 2:
			/* 如果sock被标记为写缓存用光，这时需要唤醒异步等待的进程	*/
...
int sock_awaitconn(struct socket *mysock, struct socket *servsock, int flags)
{
	...
	mysock->next = NULL;
	cli();
	/* 把客户端套接字(mysock)插入服务端的未完成队列(iconn)中，表示客户端正在等待连接 */
	 ...
	 */
	/* 唤醒服务端进程，来处理本地客户端连接
	 * 服务端进程，因为accept调用而阻塞，挂入监听socket的等待队列中
	 * 此时，有客户端连接加入，从而需要唤醒在监听socket上等待的服务端进程
	 */
	wake_up_interruptible(servsock->wait);
	/* 唤醒servsock结构中fasync_list队列中每个元素对应的进程 */
	sock_wake_async(servsock, 0);

	/* UNIX域，服务端进程处理客户端连接后，会修改客户端连接状态 *
	 * 因为服务端进程唤醒需要时间，因此if条件通常为true(即:服务端还没来得及处理该客户连接)	*/
	if (mysock->state != SS_CONNECTED) 
	{
		...
		/* 等待服务端进程处理本次连接请求
		 * 处理完成后，服务端进程会修改mysock的状态
		 * 状态变化，会激发通知事件，从而唤醒阻塞在mysock上的客户端连接
		 */
 ...
 */
/* 
 * sock_socketpair只适用与UNIX域，用于在两个进程间通过套接字建立一个连接进行数据传送
 * 这个函数用于本机范围内模拟网络方式进行进程间通信
 */
 ...
 *	ready for listening.
 *  通常情况下，函数会委托具体协议完成listen的实际操作，我们仅仅标记socket层次中几个表示socket状态的标记
 */
...
static int sock_listen(int fd, int backlog)
{
	...
	/* 要求socket的状态是SS_UNCONNECTED，刚建立状态 */
 ...
 *
 *  以"非阻塞"方式调用了accpt函数后，在使用新连接发送/接收数据之前，
...
static int sock_accept(int fd, struct sockaddr *upeer_sockaddr, int *upeer_addrlen)
{
	...
	if (sock->state != SS_UNCONNECTED) 	// 监听套接字一定是SS_UNCONNECTED状态
	{
		return(-EINVAL);
	}
	if (!(sock->flags & SO_ACCEPTCON)) 	// 检查监听套接字标记
	{
		return(-EINVAL);
	}
	...
	/* 这里的处理过程跟sock_socket()中处理过程类似	*/
	newsock->type = sock->type;    // newsock必须和监听套接字使用相同的模式
	newsock->ops = sock->ops;	// newsock必须和监听套接字属于同一个域

	/* 通过监听套接字sock获得传输层协议类型	*/
 ...
 */
/**
 * 每个协议簇定义自己的proto_ops操作集合
 * 在协议簇初始化时，使用socket_register向全局数组pops注册
 * 使用proto_ops::family作为查找标志来找到协议簇
 */
...
{
	extern struct net_proto protocols[];	/* Network protocols Protocals.c中定义的静态数组*/
